...
public class DictZipFile {
    ...
    private void _read_gzip_header() throws Exception {
        ...
        if ((flag & FEXTRA) != 0) {
            ...
            int ext = 0;
            while (true) {
                int l = ((int) extra[ext + 2] & 0xff)
                    + (256 * ((int) extra[ext + 3] & 0xff));
            ...
            this.chlen = ((int) extra[ext + 6] & 0xff)
                + (256 * ((int) extra[ext + 7] & 0xff));
            int chcnt = ((int) extra[ext + 8] & 0xff)
                + (256 * ((int) extra[ext + 9] & 0xff));
            int p = 10;
            List<Integer> lens = new ArrayList<Integer>();
            for (int i = 0; i < chcnt; i++) {
                this.chlen = ((int) extra[ext + p] & 0xff)
                    + (256 * ((int) extra[ext + p + 1] & 0xff));
                p += 2;
                lens.add(this.chlen);
            }
            int chpos = 0;
    ...
    private byte[] _readchunk(int n) throws Exception {
        ...
        InflaterOutputStream gz = new InflaterOutputStream(bos,
                                                           new Inflater(true));
        gz.write(buff);
